...
BASE_DIR = "/usr/local/airflow/include/et_data"
DOWNLOAD_DIR = os.path.join(BASE_DIR, "GEE_ET_BBOX")
RESAMPLED_DIR = os.path.join(BASE_DIR, "GEE_ET_BBOX_resampled")
...
with DAG(
    ...
    description='Monthly pipeline to download and process MODIS Evapotranspiration data from 3 months prior',
    # Run on the 1st day of each month at midnight
    schedule='0 0 1 * *',
    # Start yesterday to ensure it runs today for past data
    start_date=pendulum.yesterday(),
    catchup=False,
    tags=['et', 'evapotranspiration', 'modis', 'gee'],
) as dag:
    
    @task()
    def extract_et_data(execution_date=None):
        """
        Download MODIS ET data for three months prior to the execution date
        ...
        print(f"TASK STARTING - Downloading ET data for {year}-{month:02d}")
        ...
        print("Loading MODIS ET dataset...")
        dataset = (
        ...
        size = evapotranspiration.size().getInfo()
        ...
        def download_image(image, index):
            try:
                ...
                if response.status_code == 200:
                    ...
                    output_path = os.path.join(DOWNLOAD_DIR, f"ET_{date_str}.tif")
        ...
        try:
            ...
            list_size = et_list.size().getInfo()
    ...
    @task()
    def transform_et_data(extraction_result):
        """Transform the extracted ET data"""
        import subprocess
        import glob
        import os
        ...
        for filename in tif_files:
            ...
            # Extract date from filename (assuming pattern ET_YYYY-MM-DD.tif)
            ...
            if len(parts) > 1:
                ...
                new_filename = f"et_{year}_{month}_{day}.tif"
            else:
                # Fallback if filename doesn't match expected format
                new_filename = f"et_{base_filename}"
            ...
            # Use GDAL to resample and reproject to EPSG:4326
            cmd = [
                "gdalwarp", 
                "-t_srs", "EPSG:4326",
                "-tr", "0.02", "0.02", 
    ...
    @task()
    def load_et_data(transformation_result):
        ...
        try:
            ...
            if not partition_exists:
                ...
                cursor.execute(f"""
                CREATE TABLE et_data_{year} PARTITION OF et_data
        ...
        for filename in transformation_result['processed_files']:
            # Extract date from filename (assuming pattern et_YYYY_MM_DD.tif)
            ...
            try:
                ...
                sql_content += f"""
                INSERT INTO et_data (date_id, rast)
    ...
    # Define the workflow
    et_data = extract_et_data()
    transformed_data = transform_et_data(et_data)
    load_result = load_et_data(transformed_data)
    ...
    if pendulum.now().day > 1 and pendulum.now().month == 4:
        ...
        def extract_january_data():
            ...
            return extract_et_data(jan_date)
            
        # Execute special January processing
        jan_data = extract_january_data()
        jan_transformed = transform_et_data(jan_data)
        jan_loaded = load_et_data(jan_transformed)