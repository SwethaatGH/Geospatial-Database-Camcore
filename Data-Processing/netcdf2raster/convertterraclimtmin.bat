...
set "tmin_DIR=C:\Users\rcavalh\Documents\Camcore25\dados_empresas\Bracell\dados_cliamaticos\terraclimate\tmin"
set "TARGET_RES=0.02"
set "CUTLINE_PATH=C:\Users\rcavalh\mlogasu_hyarram\camcorecoordspolyfile\cacmcorecoordspolyfile.shp"
set "OUTPUT_DIR=%tmin_DIR%\rasters"
...
set count=0
for %%f in ("%tmin_DIR%\*.nc") do set /a count+=1
...
for %%f in (*.nc) do (
    ...
    for /L %%m in (1,1,12) do (
        ...
        set "tempfile=%OUTPUT_DIR%\terraclim_tmin_!year!_!month!_temp.tif"
        set "finalfile=%OUTPUT_DIR%\terraclim_tmin_!year!_!month!.tif"
        ...
        if exist "!finalfile!" (
            ...
            REM Try with 'tmin' variable first
            echo Command: "%GDAL_TRANSLATE%" -of GTiff -b %%m -a_srs EPSG:4326 -co COMPRESS=LZW -co TILED=YES "NETCDF:%%f:tmin" "!tempfile!"
            "%GDAL_TRANSLATE%" -of GTiff -b %%m -a_srs EPSG:4326 -co COMPRESS=LZW -co TILED=YES "NETCDF:%%f:tmin" "!tempfile!"
            ...
            if exist "!tempfile!" (
                ...
                echo Failed with 'tmin', trying 'tmin_mm'...
                "%GDAL_TRANSLATE%" -of GTiff -b %%m -a_srs EPSG:4326 -co COMPRESS=LZW -co TILED=YES "NETCDF:%%f:tmin_mm" "!tempfile!"
