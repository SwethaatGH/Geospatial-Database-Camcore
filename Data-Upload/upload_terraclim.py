"""
TerraClimate Upload Script
Appends monthly TerraClimate GeoTIFFs into existing PostGIS table with a 2025 partition.
...
# ----------------- CONFIG -----------------
INPUT_DIR = Path(r"Z:\ENVIROMICS\terraclimate\Indonesia_TerraClimate_Resampled_2025")
LOG_FILE = INPUT_DIR.parent / "upload_terraclim_log.txt"
...
def ensure_2025_partition():
    conn = psycopg2.connect(
        host=DB_HOST, port=DB_PORT, database=DB_NAME, user=DB_USER, password=DB_PASS
    )
    cur = conn.cursor()
    cur.execute(f"""
        CREATE TABLE IF NOT EXISTS {TABLE_NAME}_2025
        PARTITION OF {TABLE_NAME}
        FOR VALUES FROM ('2025-01-01') TO ('2026-01-01')
    """)
    # Clear any previously uploaded 2025 data so we start clean.
    cur.execute(
        f"DELETE FROM {TABLE_NAME} WHERE date_id >= '2025-01-01' AND date_id < '2026-01-01';"
    )
    conn.commit()
    conn.close()
    print("2025 partition ready.")
...
if __name__ == "__main__":
    ...
    with Pool(processes=NUM_WORKERS) as pool:
        for i, (status, name, msg) in enumerate(pool.imap_unordered(upload_one, tiff_files), 1):
            if status == "success":
                success += 1
                print(f"[{i}/{len(tiff_files)}] OK {name}")
            elif status == "skipped":
                skipped += 1
                print(f"[{i}/{len(tiff_files)}] SKIP {name}")
            else:
                failed += 1
                print(f"[{i}/{len(tiff_files)}] FAIL {name} - {msg}")
                failed_list.append((name, msg))

    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("TerraClimate Upload Log\n")
        f.write(f"Date: {datetime.now()}\n")
        f.write(f"Success: {success}, Skipped: {skipped}, Failed: {failed}\n\n")
        for name, msg in failed_list:
            f.write(f"FAILED: {name} - {msg}\n")
