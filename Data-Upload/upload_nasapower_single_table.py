...
# Paths
INPUT_DIR = Path(r"Q:\My Drive\Indonesia_NASA_POWER_Monthly")
...
def main():
    ...
    try:
        ...
        # Upload files variable by variable
        success_count = 0
        error_count = 0
        
        for var_idx, (var_name, files) in enumerate(sorted(files_by_var.items()), 1):
            print(f"\n{'='*60}")
            print(f"Variable {var_idx}/{len(files_by_var)}: {var_name} ({len(files)} files)")
            print(f"{'='*60}")
            
            # Progress bar for this variable
            pbar = tqdm(files, desc=f"Uploading {var_name}", unit="file")
            
            for file_path in pbar:
                if upload_file(file_path, conn):
                    success_count += 1
                else:
                    error_count += 1
