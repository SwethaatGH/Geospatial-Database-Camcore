"""
TerraClimate Indonesia 2025 Upload Script
...
LOG_FILE  = INPUT_DIR.parent / "upload_terraclim_indonesia_2025_log.txt"
...
def ensure_2025_partition():
    ...
    """)
    # Clear any previously (mis-)uploaded 2025 data so we start clean
    cur.execute(
...
if __name__ == "__main__":
    print("TerraClimate Indonesia 2025 upload")
    ...
    with Pool(processes=NUM_WORKERS) as pool:
        for i, (status, name, msg) in enumerate(pool.imap_unordered(upload_one, tiff_files), 1):
            if status == "success":
                ...
                skipped += 1
                print(f"[{i}/{len(tiff_files)}] ⏭️  {name}")
            else:
                failed += 1
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write(f"TerraClimate Indonesia 2025 Upload Log\n")
