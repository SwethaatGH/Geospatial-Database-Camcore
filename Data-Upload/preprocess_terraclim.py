"""
TerraClimate Indonesia 2025 Preprocessing Script
Converts TerraClimate NetCDF (yearly, 12 monthly bands) to monthly GeoTIFFs.
Reads from flat 2025_dataset folder (TerraClimate_<var>_2025.nc naming).
Clips to Indonesia bounding box, resamples to 0.02°, EPSG:4326.
...
INDONESIA_BBOX = (95.293026, -10.359987, 141.033852, 5.479821)  # minX, minY, maxX, maxY
...
def build_tasks():
    ...
    for nc_path in sorted(INPUT_ROOT.glob("TerraClimate_*.nc")):
        ...
        if not match:
            print(f"⚠️  Skipping (bad filename): {nc_path.name}")
            continue
...
def process_task(task):
    ...
    try:
        ...
        dst_array, dst_transform, nodata = _clip_resample(
            data, lons, lats, INDONESIA_BBOX, TARGET_RES)
...
if __name__ == "__main__":
    start = datetime.now()
    print("TerraClimate Indonesia 2025 preprocessing")
