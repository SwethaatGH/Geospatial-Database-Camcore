...
Input: ERA5_Indonesia_YYYY-MM-DD.tif files with 15 bands
...
# Configuration
ERA5_FOLDER = r"Z:\ENVIROMICS\era5\Indonesia_ERA5_Daily"
DB_CONFIG = {
    'host': 'localhost',
    'port': 5433,
...
def parse_filename(filename):
    """Extract date from filename: ERA5_Indonesia_YYYY-MM-DD.tif"""
    try:
        date_str = filename.replace("ERA5_Indonesia_", "").replace(".tif", "")
...
def get_all_files():
    ...
    files = [f for f in os.listdir(ERA5_FOLDER) if f.startswith("ERA5_Indonesia_") and f.endswith(".tif")]
    files.sort()
    return [os.path.join(ERA5_FOLDER, f) for f in files]


def ensure_partitions(years):
    """Create yearly partitions for era5_data if they don't exist."""
    conn = psycopg2.connect(**DB_CONFIG)
    cur = conn.cursor()
    for y in years:
        cur.execute(f"""
            CREATE TABLE IF NOT EXISTS era5_data_{y}
            PARTITION OF era5_data
            FOR VALUES FROM ('{y}-01-01') TO ('{y+1}-01-01')
        """)
        print(f"  Partition era5_data_{y} ready.")
    conn.commit()
    conn.close()
...
def main():
    ...
    years = sorted({int(parse_filename(os.path.basename(f))[:4]) for f in files if parse_filename(os.path.basename(f))})
    ...
    try:
        ...
        # Create new cursor outside the previous 'with' block
